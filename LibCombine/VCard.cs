...
namespace LibCombine
{
  ...
  public class VCard
  {
      ...
      : this()
    {
      ...
      if (list.Count > 0)
      {
        Email = list[0].InnerText;
      }
      else
      {
        list = element.GetElementsByTagName("hasEmail", vcardNS);
        if (list.Count > 0)
          Email = ((XmlElement) list[0]).GetAttribute("resource", "http://www.w3.org/1999/02/22-rdf-syntax-ns#");
      }
